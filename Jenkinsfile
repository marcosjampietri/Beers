            ...
            steps {
               script {
                  ...
                  withCredentials([usernamePassword(credentialsId: 'dockerhub-cred', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_ID')]) {
                      sh "docker build -t marcosjampietri/beer-next:${NEW_VERSION} ./client"
                      
                      sh "docker build -t marcosjampietri/beer-nginx:${NEW_VERSION} ./nginx"
                      
                      sh 'echo ${DOCKER_PASSWORD} | docker login -u ${DOCKER_ID} --password-stdin'
                      
                      sh "docker push marcosjampietri/beer-next:${NEW_VERSION}"
                      
                      sh "docker push marcosjampietri/beer-nginx:${NEW_VERSION}"
                  }
               }
            }
        }
            ...
            steps {
                script {                
                    ...
                    sshagent(['ansible_server_key']) {
                        ...
                        withCredentials([sshUserPrivateKey(credentialsId: 'Marcos-ec2-22', keyFileVariable: 'KEYFILE', usernameVariable: 'USER')]) {
                            sh 'scp $KEYFILE root@$ANSIBLE_SERVER:/root/Marcos-ec2-22.pem'
                        }
                    }
                }
            }
